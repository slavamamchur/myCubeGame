...
public class DBTableFragment extends Fragment {
    ...
    private void initHeaders() {
        ...
        if (columns != null)
            for (DBColumnInfo column : columns) {
                ...
                caption.setText(column.getType().equals(DBColumnInfo.ColumnType.COLUMN_BUTTON) ? "" : column.getCaption());
                caption.setTextSize(18);
                caption.setTypeface(null, Typeface.BOLD);
                if (!column.getType().equals(DBColumnInfo.ColumnType.COLUMN_BUTTON) && column.getIconResId() > 0)
    ...
    private void fillHolder(ArrayList<View> holder, final Object item) {
        ...
        for (int i = 0; i < holder.size(); i++) {
            ...
            if (DBColumnInfo.ColumnType.COLUMN_BUTTON.equals(columns.get(i).getType()) || DBColumnInfo.ColumnType.COLUMN_REFERENCE.equals(columns.get(i).getType()))
    ...
    private LinearLayout createRowView(int pos) {
        ...
        if (columns != null)
            for (DBColumnInfo column : columns) {
                ...
                View col;
                if (column.getType().equals(DBColumnInfo.ColumnType.COLUMN_CHECK_BOX)) {
                    col = new CheckBox(getContext());
                    ((CheckBox)col).setGravity(Gravity.CENTER);
                }
                else if (!column.getType().equals(DBColumnInfo.ColumnType.COLUMN_BUTTON)) {
    ...
    public void fillColumnData(View view, DBColumnInfo column, Object item) {
        try {
            ...
            else if (column.getType().equals(DBColumnInfo.ColumnType.COLUMN_COLOR_BOX))
                view.setBackgroundColor(0xFF000000 | (Integer) value);
            else
                if (!column.getType().equals(DBColumnInfo.ColumnType.COLUMN_BUTTON)) {
