...
public class ShadowBox {
    ...
    public ShadowBox(Matrix4f lightViewMatrix, GLCamera camera, float aspectRatio) {
    ...
    private void calculateWidthsAndHeights() {
        farWidth = (float) (SHADOW_DISTANCE * Math.tan(Math.toRadians(GLCamera.VERT_FOV)));
        nearWidth = (float) (GLCamera.NEAR_PLANE * Math.tan(Math.toRadians(GLCamera.VERT_FOV)));
    ...
    protected Vector3f getCenter() {
        ...
        invertedLight.invert(lightViewMatrix);
    ...
    private Vector4f[] calculateFrustumVertices(Matrix4f rotationM, Vector3f forwardVector, Vector3f centerNear, Vector3f centerFar) {
        Vector3f upVector = new Vector3f(UP);
        rotationM.transform(upVector);
